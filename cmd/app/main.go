...
func main() {
	...
	// 3rd party services
	r2 := storage.NewCloudflareR2(ctx, cfg)
	emailClient := email.NewClient(ctx, cfg)
	smsClient := sms.NewClient(ctx, cfg)
	...
	offenderService := offender.NewService(cfg, offenderRepo, r2)
